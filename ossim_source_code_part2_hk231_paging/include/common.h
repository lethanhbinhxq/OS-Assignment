...
#define ADDRESS_SIZE	20 //// address size = 5 bit segment + 5 bit page + 10 bit offset
...
#define NUM_PAGES	(1 << (ADDRESS_SIZE - OFFSET_LEN)) //// numPage = 2^10
#define PAGE_SIZE	(1 << OFFSET_LEN) //// pageSize = 2^10

enum ins_opcode_t { //// five types of instruction executed by CPU
...
struct inst_t {
	enum ins_opcode_t opcode; //// type of instruction
...
struct code_seg_t {
	struct inst_t * text;
	uint32_t size; //// size of code segment
};
